...
- **Service lifecycle** — native OS service on **Linux interactive-desktop** (systemd **user** unit under an operator session; runs as the **logged-in operator user** in V1 because the keystore dependency forbids a separate system account; system-unit / headless-boot is V1.5+ pending a TPM-backed or keyctl-based secret backend per secrets-bridge spec), **macOS** (launchd system daemon under a dedicated `_mothership` service account), **Windows** (Windows service under a dedicated virtual service account); no agent-reachable control surface.
...
- **Plugin install orchestration** — supervisor drives the `reviewed` → `staged` → `committed` → `activated` transitions of the plugin-bundle install state machine (sub-spec 3); operator approval is **journaled** immediately after re-auth (durable HMAC-chained record in the install journal, not a distinct audit event); the canonical `plugin.install-approved` audit event (declared by the plugin-bundle spec) is emitted at `activated`; owns operator-facing review UI; persists approved grants to the ledger.
...
|----|-----------|-----------------|-------------|
| Linux | systemd **user** unit `mothership-supervisor.service` under the active operator session | Logged-in operator user (no separate dedicated system account in V1 — the keystore dependency forbids it) | `systemctl --user enable`; starts when the operator's user manager / session is active; `Restart=on-failure` with backoff |
...
The supervisor **never runs as root / Administrator / LocalSystem**. On Linux V1 it runs as the logged-in operator user specifically so it can use the session D-Bus / libsecret keystore facilities; on macOS and Windows it runs under the dedicated non-privileged service account listed above. The runtime identity has only the rights needed for its specific duties (bind to its local IPC path, read/write its persistent state directory, call into the OS keystore for its declared handles, and spawn broker and skill processes under the intended per-org-context identity model). On Linux, local runtime IPC paths live under `$XDG_RUNTIME_DIR/mothership/` (not `/var/run/`); persistent state lives under `$XDG_DATA_HOME/mothership/` or operator-configured equivalent; this spec's path examples using `/var/run/` and `/var/lib/` apply to macOS / Windows equivalents and are a Linux-user-session mapping on Linux V1.

### Control surface

Operator-facing CLI: `mothership` binary (separate process; not the supervisor). CLI → supervisor communication is over a **unix domain socket** (Linux/macOS) or a **named pipe** (Windows) owned by the runtime identity with peer-credential-check on connect. On Linux V1, the unix socket lives under `$XDG_RUNTIME_DIR/mothership/supervisor.sock`; on macOS it lives under `/var/run/mothership/supervisor.sock`. No TCP/IP listener. No agent-reachable socket. The gateway (sub-spec 7) is a separate process that does handle external browser-origin requests; the supervisor does not.
...
On Linux/macOS the supervisor is a non-root user but needs to spawn skill processes under per-org-context unix users. A minimal **privileged-spawn helper** (`mothership-spawnd`) is the only component in the **trusted computing base (TCB)** authorized to call `setuid(target_uid)`. It accepts requests exclusively over an authenticated IPC channel (a pre-inherited pipe from the supervisor) carrying structured messages; **it is not exposed as an argv-driven CLI to any caller**. The helper binary's `main()` refuses to start if invoked with any argv beyond its own binary name, and refuses to operate if its authenticated-IPC fd is absent. The message schema is:

```
SpawnRequest {
  org_context: str,          // resolved authoritatively via signed org-contexts.yaml
  bundle_hash: [u8; 32],
  skill_id: str,             // normative id per bundle-identity grammar
  exec: str,                 // absolute canonical path within bundles/<bundle_hash>/
  stdin_fd:  fd_descriptor,  // passed via SCM_RIGHTS on the authenticated IPC
  stdout_fd: fd_descriptor,
  stderr_fd: fd_descriptor,
  broker_control_fd: fd_descriptor,
  challenge_nonce: [u8; 32], // supervisor-issued nonce for this spawn
}
```

- **Authenticated input.** Helper only accepts messages over the supervisor-inherited IPC channel, authenticated by a per-session nonce issued at supervisor start and held only in supervisor memory. **The helper has no argv surface and no environment-variable input surface; the only way to invoke it is via its pre-inherited IPC fd.**
- **Target-UID derivation (not accepted as input).** The helper loads `/etc/mothership/org-contexts.yaml` (signed by operator GPG) at start; the `target_uid` and `target_gid` are **derived from the signed `org_context` entry**, never accepted as caller-provided fields. An attempt to provide uid/gid on the request is rejected with `spawn-rejected(reason: uid-supplied-by-caller)`. This closes the class of attack where a compromised caller could misdirect the helper to an unlisted UID.
- **Allowed org_context allowlist.** `org_context` must be exactly one of the declared contexts in the signed registry, or the reserved `broker` context. Any other value → `spawn-rejected(reason: org-context-unknown)`.
- **Allowed exec path.** `exec` must be an absolute path rooted under `/var/lib/mothership/bundles/<bundle_hash>/` (Linux V1: `$XDG_DATA_HOME/mothership/bundles/<bundle_hash>/` equivalent). Helper verifies the path is canonical, a regular file, the `bundle_hash` is in the active registry (cross-checked by calling back to supervisor over the same IPC — single round-trip), and the path's mode matches supervisor-imposed-at-commit modes.
...
| `/etc/mothership/org-contexts.yaml` | Org-context registry (used for org-context isolation + privileged-spawn allowlist resolution + unix-user mapping) | Yes (operator GPG) | Re-auth + re-sign required |
...
| `/etc/mothership/operator.pub` | Pinned operator public GPG key (trust anchor for all of the above) | No — **trust anchor**, pinned at install | Install-time pin or `mothership operator rotate-key` (re-auth gated) only |
...
1. OS service manager starts `mothership-supervisor` under the configured runtime identity for the platform (Linux V1: logged-in operator user; macOS: dedicated `_mothership` service user; Windows: dedicated virtual service account under the `MothershipSupervisor` service SID).
...
13. Spawn the broker process (sub-spec 2 defines the broker launch contract) with the supervisor socket as its only control-plane channel. Pass the supervisor's current policy snapshot + manifest registry + grant snapshot via that channel. **Audit writes from the broker follow the audit-log spec's access-control model**: the supervisor opens the audit log file and passes the specific audit-write fd to the broker at spawn via fd inheritance (Linux/macOS) or `DuplicateHandle` (Windows), per the audit-log spec's capability-reference contract. A generic "audit writer handle" over the supervisor socket is explicitly **not** the transport. Alternatively, the broker may route audit events back to the supervisor's in-process `AuditWriter` via the supervisor socket; sub-spec 2 picks between the two and this spec accepts either. Broker refuses to start without these inputs + whichever audit path the audit-log contract selects.
...
3. **Activate.** Supervisor receives ack. Records activation in the journal, including `snapshot_version`; emits the relevant audit event using **that event's plugin-bundle-/audit-log-declared shape only** — this sub-spec does not extend a canonical cross-spec event with supervisor-owned fields. For events whose shape this sub-spec owns (policy, grants), the `snapshot_version` is a field of the event payload; for events whose shape is owned by another sub-spec (notably `plugin.install-approved`), snapshot correlation lives in the supervisor journal keyed by `install_event_id`, and in the broker's intent events which independently carry `snapshot_version` per the cross-spec normative requirement below. Supervisor then serves the read APIs (`GET policy/current`, `GET grants/current`) from the new snapshot. Older reads continue to see the prior snapshot until the transition completes.
...
3. **Emit chain-restart marker.** Write a new grant-ledger entry with `op: "grants.chain-restart"`. The closed `op` union for the grant-ledger schema **must include `"grants.chain-restart"` as an explicit variant** (alongside `grant-create-tentative`, `grant-create`, `grant-create-compensated`, `grant-revoke`, `grant-use-recorded`) so codegen treats it as part of the closed set. This variant uses the **same top-level ledger-entry shape** as the rest of the grant ledger, including the standard `authority` / `actor` / timestamp fields; the re-auth reference is recorded at `authority.reauth_challenge_id`, not as a top-level field, to keep the schema uniform. Required fields specific to this variant are: `op`, `restart_reason: "operator-recover" | "orphan-tentative-reconciliation" | "hmac-corruption"`, `prior_closing_mac`, `new_genesis_mac`, and `new_key_epoch` (incremented from the prior chain); `authority.reauth_challenge_id` MUST be populated as for other re-auth-gated ops. This entry is the first record of the post-recovery chain.
...
5. **Operator choice: replay-from-audit or clean-slate.**
   - **Replay-from-audit** (default when feasible): the supervisor walks the audit log's `plugin.install-approved` events (canonical event from the plugin-bundle spec, emitted at `activated`) from the earliest install forward, uses each event's `grants_approved` payload as the authoritative approved-grants list, and re-creates grant-ledger entries matching the recorded approval. Each replayed grant is annotated `replay_source: audit_seq:<n>` so the operator can distinguish original from replayed.
...
| `reviewed` | Present capability-review UI over the CLI or gateway. Require re-auth via sub-spec 7. On **successful** re-auth + operator approval: **persist the approval decision durably to the supervisor install journal** (`journal/<install_event_id>.jsonl`) with full grants-approved list, operator identity fingerprint, and `reauth_challenge_id`. Do **NOT** emit `plugin.install-approved` at this state — that canonical audit event is reserved for `activated` (see plugin-bundle spec's declared install-event shapes, which this sub-spec consumes as-is without redefinition). On timeout/rejection → emit `plugin.install-rejected`. |
| `staged` | Supervise payload materialization in `installs/<install_event_id>/payload/` (per plugin spec). No grant mutation yet. |
| `committed` | Two-phase commit via §"State versioning and broker coordination": **Prepare** — atomically rename payload to `bundles/<bundle_hash>/`; write grant-create entries to the grant ledger (tentative — see below); mutate the manifest registry in a staging buffer; build new `SupervisorSnapshot`. **Broker-ack** — push new snapshot; await ack. On **ack**, commit grant-ledger entries (promote tentative to durable). On **timeout/reject**, compensation: revert manifest registry; **write grant-ledger compensation records** with `op: "grant-create-compensated"` for each tentative grant (records reference the original tentative seq range); if an audit entry is emitted for this rollback, the **audit event name** is `policy.grant-create-compensated` — distinct from the grant-ledger `op` value, though intentionally homonymous for operator legibility; rollback payload rename to `rolled-back-bundles/<bundle_hash>/`; emit `plugin.install-rejected(reason_code: broker-ack-failed)` audit event. |
| `activated` | Emit the canonical `plugin.install-approved` audit event **using the plugin-bundle spec's declared shape only** — this sub-spec does **not** add `snapshot_version` or `approval_journal_seq` to that audit payload. The activation's `snapshot_version` and the back-reference to the reviewed-stage install-journal approval record (`approval_journal_seq`) are recorded in the supervisor install journal keyed by `install_event_id`, not in the canonical audit event. This state marks the point after which broker admits intents naming this bundle's skills/agents. |

### Why approval decision is journaled at `reviewed`, not audit-logged

Codex flagged the risk that if the install fails between `reviewed` and `activated`, the operator's approval decision never becomes durable — a P8 hole. Durability is preserved here by writing the approval to the **supervisor install journal** (`journal/<install_event_id>.jsonl`) at `reviewed`, NOT by emitting a new audit event. Journal entries are tamper-evident (HMAC-chained per the audit log's approach, keyed to `mothership/journal/hmac`) and are the authoritative source the recovery algorithm replays from. This preserves the existing canonical plugin-bundle install-event-audit taxonomy (`plugin.install-approved` emitted only at `activated`, with the plugin-bundle-declared payload shape) without introducing a new audit event that would conflict with the already-merged plugin-bundle spec. **`snapshot_version` and `approval_journal_seq` are supervisor-journal metadata / back-references, never fields on the canonical `plugin.install-approved` audit event** — the audit event's shape is owned by the plugin-bundle spec and is not extended here. If activation fails after review, the durable sequence is: journal approval record (durable, HMAC-chained) → `plugin.install-rejected(reason: broker-ack-failed)` audit event (durable). Forensic analysis joins the two via `install_event_id`.
...
A fixed-path memory-mapped file at `<runtime_dir>/mothership/status` — on Linux V1 `$XDG_RUNTIME_DIR/mothership/status`; on macOS `/var/run/mothership/status`; on Windows a named shared-memory object under the service SID — holds a tiny struct updated at every state transition: `{version, state: "starting"|"ready"|"degraded"|"audit-unavailable"|"stopping"|"stopped", last_success_ts, last_error, last_error_ts}`. Read by `mothership status` CLI without any broker / audit dependency. This is the out-of-band signal path referenced by the audit-log spec for `audit-unavailable` notifications.
...
**Service lifecycle:**
- Unit starts under the configured runtime identity per platform (logged-in operator user on Linux V1; dedicated `_mothership` service user on macOS; virtual service account on Windows); fails to start as root / Administrator / LocalSystem with a diagnostic; `NoNewPrivileges` verified via `/proc/<pid>/status` on Linux.
...
**Happy path:**
- Full install flow: plugin-bundle spec's end-to-end install runs through the supervisor; all six plugin install states transition correctly; reviewed-stage journal record is durably written; `plugin.install-approved` audit event is emitted at `activated` **using its plugin-bundle-declared payload shape** (no `snapshot_version` or `approval_journal_seq` added to the event); correlation to the reviewed-stage approval and activated snapshot is verified via `install_event_id` plus supervisor journal metadata (reviewed-stage journal seq + snapshot linkage recorded at activation), not by asserting non-canonical audit payload fields; grant ledger reflects approved capabilities with promoted-from-tentative entries; broker's `broker.snapshot-accepted` ack is recorded.
- Plugin bundle install + use: install a test bundle → grant capability → broker admits corresponding intent → skill executes with declared caps → audit chain correlates install, grant, and intent via `install_event_id`/grant identifiers together with supervisor snapshot/journal metadata and with the broker's own intent events (which do carry `snapshot_version` per the cross-spec normative requirement on sub-spec 2), without requiring every canonical audit event payload to carry `snapshot_version`.
...
- **Broker ack succeeds but supervisor crashes before activate-audit.** On restart, supervisor replays from journal; sees ack recorded; replays activation; emits `plugin.install-approved` after restart (using its plugin-bundle-declared shape). Audit chain is contiguous; snapshot correlation is recovered via the supervisor journal keyed by `install_event_id` together with the broker's intent events (which carry `snapshot_version`), not via extra fields on the canonical install-approved event.
...
**Normative:** every blocked attempt MUST produce a correlated audit entry. Two enforcement paths exist; both must be auditable:

- **Broker-mediated intent path.** An operation expressed as a broker-routed intent is rejected before any protected syscall fires on the agent's behalf. Audit: `broker.intent-denied` carrying actor, intent, provenance, matrix rule that fired. This is the common case — agents operate through intents, not direct syscalls.
- **Direct-syscall-bypass path.** A compromised agent attempting to bypass the broker by issuing raw syscalls is denied by OS permissions + sandbox policy (seccomp on Linux, sandbox-exec on macOS, service-restricted-token on Windows). When the sandbox observation layer (Linux seccomp user-notify or equivalent) attributes the attempt to a Mothership actor, the broker emits `broker.direct-syscall-observed`. This path depends on the sandbox providing attribution — sub-spec 4 (Skill process isolation) is the authoritative source for what is observable.

OS-level `EACCES` alone, without sandbox-observation attribution, provides enforcement but no forensic visibility. This acceptance test requires the attribution path to exist for every scenario below or the scenario is not testable.
...
1. **Policy file write attempt.** Two blocked paths in scope: (a) if the agent emits a broker-mediated fs-write intent targeting `/etc/mothership/policy.yaml`, broker evaluates against the matrix and emits `broker.intent-denied` before any syscall; (b) if the compromised agent calls `open(…, O_WRONLY)` directly, OS/sandbox denies and the sandbox observation path emits `broker.direct-syscall-observed` when the bypass is attributed.
2. **Policy reload via supervisor socket from agent credentials.** Agent tries to connect to the supervisor's local IPC socket (Linux: `$XDG_RUNTIME_DIR/mothership/supervisor.sock`; macOS: `/var/run/mothership/supervisor.sock`; Windows: named pipe). Peer-credential check rejects. Audit: `supervisor.peer-credential-rejected` with agent's observed UID/org_context (Linux/macOS) or SID (Windows).
...
4. **Grant self-create via ledger-file direct write.** Same split as #1: broker-mediated append intent targeting `/var/lib/mothership/grants/ledger.jsonl` → `broker.intent-denied`; direct `open(…, O_APPEND)` → OS/sandbox denies; sandbox observation path emits `broker.direct-syscall-observed` when attributed.
5. **Audit log direct write.** Same split as #4 against the audit log.
6. **Audit log fs race / overwrite.** Broker-mediated rename/unlink intent → `broker.intent-denied`. Direct filesystem syscall bypass → OS/sandbox denies, `broker.direct-syscall-observed` when attributed. IPC-targeted race (agent tries to hijack the exclusive write lock via supervisor socket) → `supervisor.peer-credential-rejected`.
7. **Plugin install bypass.** Broker-mediated write intent into `/var/lib/mothership/bundles/<bundle_hash>/` → `broker.intent-denied`. Direct filesystem write → OS/sandbox denies; `broker.direct-syscall-observed` when attributed. Even if OS perms were wrong, supervisor's registry rejects unregistered `bundle_hash` directories at next snapshot build.
...
9. **Config signature strip.** Broker-mediated unlink intent on `.sig` file → `broker.intent-denied`. Direct unlink → OS/sandbox denies; `broker.direct-syscall-observed` when attributed. Supervisor's loader requires `.sig` present — missing-sig at next boot/reload is `config-signature-missing`, never "assume unsigned."
10. **Status-pipe forgery.** Broker-mediated write intent on the status pipe → `broker.intent-denied`. Direct write → OS/sandbox denies; `broker.direct-syscall-observed` when attributed.
11. **Broker-control channel injection.** Agent tries to send messages on the supervisor→broker control channel. Broker authenticates messages via the broker-control HMAC key held only by supervisor; agent's forged messages fail HMAC verification. Audit: the broker's control-channel-auth-failure event from its own event registry (name owned by sub-spec 2; the supervisor spec does not declare a `broker.*` event on its behalf).
...
- **Supervisor audit event shapes** (declared here for the audit-events registry):
  ...
  - Grants: `policy.grant-created` (post-ack promotion), `policy.grant-create-tentative` (Prepare phase), `policy.grant-create-compensated` (ack-failed or orphan reconciliation), `policy.grant-revoked`, `policy.grant-use-recorded`
  - Plugin install: reviewed-stage approval decision is persisted to the supervisor install journal (NOT a distinct audit event); the canonical `plugin.install-approved` audit event defined in the plugin-bundle spec is emitted at `activated` **using the plugin-bundle-declared payload shape only**. Correlation to the supervisor snapshot and the reviewed-stage journal entry is maintained as **supervisor-owned metadata keyed by `install_event_id`** (not as extra fields on the canonical audit event); if independently auditable snapshot correlation is needed at some later point, it will be added via a **separate supervisor-owned audit event**, not by extending `plugin.install-approved`. No new plugin.install-* audit events are introduced here.
...
- [ ] Spec-specific acceptance test passes (all 20 L4 compromised-agent scenarios in CI; no state-mutation side effect persists in any case)
...
Codex's review was substantively correct on every structural point; the V0 draft would have shipped with a supervisor-broker coordination architecture that would fail on the first broker restart mid-install. The revision adds two major new primitives — `SupervisorSnapshot` with two-phase commit, and the privileged-spawn helper as a designed TCB component — plus a broader monotonic-lock baseline that closes the provenance-class-downgrade holes. Approval-durability placement, grant-ledger recovery algorithm, and the interactive-Linux V1 scope decision resolve three distinct architectural ambiguities. The resulting spec is materially more coherent and harder to exploit; it now specifies, rather than gestures at, every supervisor-broker coordination point. Cross-spec impact on sub-spec 2 (Broker) is minor but normative: broker events MUST carry `snapshot_version` and broker's compiled matrix MUST be snapshot-version-tied.

---

## Amendments log (continued)

| Date | Change | Reason |
|------|--------|--------|
| 2026-04-19 | V0.2 Copilot review reconciliation. | Fixed 8 issues on PR #2, including one cross-spec inconsistency that the V0.1 draft introduced: (a) **reverted invented `plugin.install-approved-decision` + `plugin.install-activated` event names** that conflicted with the already-merged plugin-bundle spec's canonical `plugin.install-approved` at `activated`. Durable approval at `reviewed` is now achieved by persisting the decision to the supervisor install journal (`journal/<install_event_id>.jsonl`, HMAC-chained via `mothership/journal/hmac`) — NOT by emitting a new audit event. The canonical `plugin.install-approved` audit event defined in the plugin-bundle spec is the only install-approval audit event, emitted at `activated`, now carrying `snapshot_version` + `approval_journal_seq`. Forensic analysis joins journal record and audit event via `install_event_id`. This preserves the P8 durability property (approval survives activation failure) without creating an event-name conflict across specs; (b) Linux service model reconciled — V1 is a systemd **user** unit under an active operator session (runs as logged-in operator user because keystore dependency forbids a separate system account in V1); `/var/run/` path examples are Linux-user-session-mapped to `$XDG_RUNTIME_DIR/mothership/` and `/var/lib/` to `$XDG_DATA_HOME/mothership/`; (c) signed-config file table expanded to include `/etc/mothership/org-contexts.yaml` (referenced by spawn helper) and `/etc/mothership/operator.pub` (the pinned trust anchor, separately integrity-protected, not operator-signed); (d) Linux keystore cross-reference updated to point at the secrets-bridge spec (authoritative source) rather than the plugin-bundle spec; (e) compensation audit event on broker-ack failure is `policy.grant-create-compensated` (matches declared grant-ledger event type), not `policy.grant-revoked`; (f) removed stray `policy.approval-decision` from the event list (not referenced anywhere; would have created a second "approval decision" concept in the audit registry); (g) exit checklist updated from "all 12 L4 scenarios" to "all 20 L4 scenarios" matching the current count; (h) L4 scenario #1 and related (#4, #5, #6, #7, #9, #10) rewritten to explicitly split broker-mediated-intent-denial from direct-syscall-bypass-denial paths; `broker.direct-syscall-observed` is only emitted when the sandbox observation layer (Linux seccomp user-notify or equivalent) can attribute the attempt to a Mothership actor — cross-referenced to sub-spec 4 as authoritative on observability. No contract changes to downstream sub-specs. |
| 2026-04-20 | V0.4 third-round Copilot reconciliation. | Fixed 6 more issues on PR #2. Five are the same pattern I keep tripping over — I removed the `snapshot_version` / `approval_journal_seq` claim from the main `plugin.install-approved` definition in V0.3 but missed **five downstream references** that still asserted the canonical audit event carried those fields. This V0.4 fix purges every remaining claim: (a) §"State versioning and broker coordination" Activate step now says supervisor-owned events carry `snapshot_version` in their own payload; cross-spec events (notably `plugin.install-approved`) use only their declared shape, with correlation via supervisor journal keyed by `install_event_id` and the broker's intent events that independently carry `snapshot_version`; (b) L3 happy-path integration test no longer asserts the canonical event carries supervisor-owned fields; (c) L3 broker-ack-crash test clarifies correlation comes from journal + broker intent events; (d) audit-event list Plugin install bullet says correlation is supervisor-owned metadata keyed by `install_event_id`, with a separate supervisor-owned audit event possible if independently-auditable correlation is needed later; (e) `committed` state row disambiguates grant-ledger `op: "grant-create-compensated"` (a ledger record field) from the audit event name `policy.grant-create-compensated` (homonymous for operator legibility but distinct concepts); (f) grant-ledger `grants.chain-restart` variant field layout aligned to the rest of the ledger schema — `authority.reauth_challenge_id` instead of top-level `reauth_challenge_id`, so codegen has no special-case handling. No contract changes to downstream sub-specs. |
| 2026-04-19 | V0.3 second-round Copilot reconciliation. | Fixed 13 more issues on PR #2. Two are the SECOND round of cross-spec-audit-event conflicts — the V0.2 fix kept saying the canonical `plugin.install-approved` at `activated` "carries `snapshot_version` + `approval_journal_seq`," but those fields are not in the plugin-bundle spec's declared payload shape and the audit-log spec enforces schema validation. This V0.3 fix **removes `snapshot_version` and `approval_journal_seq` as fields on the audit event entirely**; they are now supervisor-journal metadata keyed by `install_event_id`. The canonical `plugin.install-approved` audit event continues to use the plugin-bundle-declared payload without extension. Cross-spec forensic correlation is via `install_event_id` only. Other fixes: (a) Phase 6 audit-writer-handle-over-socket removed — broker gets audit-write fd via inheritance/dup per the audit-log spec's capability-reference contract, or routes audit events back to the supervisor's in-process `AuditWriter`; sub-spec 2 picks between these; (b) replay-from-audit now uses `plugin.install-approved.grants_approved` directly (no journal cross-reference needed since the canonical event already carries the approved list); (c) grant-ledger `op` union explicitly includes `grants.chain-restart` as a declared variant so codegen treats it as part of the closed set; (d) spawn helper: removed caller-supplied `--target-uid / --target-gid`; target uid/gid is **derived from the signed `org-contexts.yaml` entry based on `org_context`**, never accepted as input — closes the "misdirect to an unlisted UID" attack class; (e) spawn helper has structured-IPC message schema with no argv surface — helper binary refuses to start if invoked with argv beyond its own name; (f) L4 #11 broker-control-auth audit event name owned by sub-spec 2 (not declared here as `broker.peer-credential-rejected`); (g) status-pipe path parameterized as `<runtime_dir>/mothership/status` with per-platform values (Linux V1 `$XDG_RUNTIME_DIR`, macOS `/var/run`, Windows named shared memory); (h) Scope bullet for service lifecycle now explicitly notes Linux V1 runs as logged-in operator user vs. macOS/Windows dedicated service accounts; (i) Scope bullet for plugin install now says approval is journaled, audit at `activated` (matches the detail section); (j) Codex Review wrong-assumptions-finding bullet updated to describe the V0.2 reconciled design, not the V0.1 invented event names; (k) Phase 1 now says "configured runtime identity for the platform" instead of "dedicated service account"; (l) L2 test bullet likewise; (m) "trust computing base" → "trusted computing base (TCB)". No contract changes to downstream sub-specs. |
