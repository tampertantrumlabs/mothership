...
3. **Emit chain-restart marker.** Write a new grant-ledger entry with `op: "grants.chain-restart"`. The closed `op` union for the grant-ledger schema **must include `"grants.chain-restart"` as an explicit variant** (alongside `grant-create-tentative`, `grant-create`, `grant-create-compensated`, `grant-revoke`, `grant-use-recorded`) so codegen treats it as part of the closed set. Required fields for this variant are: `op`, `reauth_challenge_id`, `restart_reason: "operator-recover" | "orphan-tentative-reconciliation" | "hmac-corruption"`, `prior_closing_mac`, `new_genesis_mac`, and `new_key_epoch` (incremented from the prior chain). This entry is the first record of the post-recovery chain.
...
- **Broker ack succeeds but supervisor crashes before activate-audit.** On restart, supervisor replays from journal; sees ack recorded; replays activation; emits `plugin.install-approved` after restart. Audit chain is contiguous and referencing the right `snapshot_version`.
...
- **Supervisor audit event shapes** (declared here for the audit-events registry):
  ...
  - Plugin install: reviewed-stage approval decision is persisted to the supervisor install journal (NOT a distinct audit event); the canonical `plugin.install-approved` audit event defined in the plugin-bundle spec is emitted at `activated` and carries `snapshot_version` + `approval_journal_seq`. No new plugin.install-* audit events are introduced here.
