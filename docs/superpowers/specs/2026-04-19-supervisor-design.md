...
5. **Operator choice: replay-from-audit or clean-slate.**
   - **Replay-from-audit** (default when feasible): the supervisor walks the audit log's `plugin.install-approved` events (canonical event from the plugin-bundle spec, emitted at `activated`) from the earliest install forward, cross-referenced with each install's reviewed-stage journal record for the full grants-approved list, and re-creates grant-ledger entries matching the recorded approval. Each replayed grant is annotated `replay_source: audit_seq:<n>` so the operator can distinguish original from replayed.
...
| `activated` | Emit the canonical `plugin.install-approved` audit event (shape defined in plugin-bundle spec) with `snapshot_version`, `bundle_hash`, and a back-reference to the reviewed-stage install-journal approval record (`approval_journal_seq`). This event marks the point after which broker admits intents naming this bundle's skills/agents. |

### Why approval decision is journaled at `reviewed`, not audit-logged

Codex flagged the risk that if the install fails between `reviewed` and `activated`, the operator's approval decision never becomes durable — a P8 hole. Durability is preserved here by writing the approval to the **supervisor install journal** (`journal/<install_event_id>.jsonl`) at `reviewed`, NOT by emitting a new audit event. Journal entries are tamper-evident (HMAC-chained per the audit log's approach, keyed to `mothership/journal/hmac`) and are the authoritative source the recovery algorithm replays from. This preserves the existing canonical plugin-bundle install-event-audit taxonomy (`plugin.install-approved` emitted only at `activated`) without introducing a new audit event that would conflict with the already-merged plugin-bundle spec. If activation fails after review, the durable sequence is: journal approval record (durable, HMAC-chained) → `plugin.install-rejected(reason: broker-ack-failed)` audit event (durable). Forensic analysis joins the two via `install_event_id`.
