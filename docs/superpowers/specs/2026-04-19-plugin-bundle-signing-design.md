...
Define the **install unit** of Mothership: a signed archive that declares everything the supervisor and broker need to sandbox, authorize, and audit extensions. This sub-spec guarantees **manifest integrity and install-time verification** for P3 and P7: **an attacker with a fresh, unverified identity cannot publish a skill that reads user secrets on a default-configured Mothership, and any signed bundle must exactly match its declared manifest and capabilities at install time.** Runtime enforcement that a signed skill cannot exceed its declared manifest at execution time is owned by sub-spec 4 (T4 — Skill process isolation); both halves must hold for P3 end-to-end.
...
- `bundle_id` MUST be exactly `<publisher_id>/<local_name>`, where `<local_name>` independently matches the same grammar as `publisher_id`. The separator is a single ASCII `/`; no other Unicode slashes are permitted. **Normatively: the first segment of `bundle_id` MUST equal the standalone `publisher_id` field by exact byte comparison**; neither field overrides the other and any mismatch is invalid.
...
The verifier at `descriptor-verified` MUST reject — with `manifest-schema-invalid` — any descriptor whose `publisher_id` or `bundle_id` contains non-ASCII characters, uppercase letters, Unicode normalization variants, visually-confusable characters, extra path separators, empty segments, or otherwise fails the grammar. The verifier MUST also split `bundle_id` on the required single ASCII `/` and reject the descriptor if its first segment is not exactly equal to `publisher_id` by byte comparison — neither field is authoritative over the other, both must agree. Because only ASCII lowercase is admissible, there is no valid mixed-case form to normalize; invalid syntax is **rejected**, not normalized. Build tooling MUST emit only this form and MUST fail generation (rather than silently choosing one field as authoritative) if `publisher_id` and `bundle_id`'s first segment differ.
...
- **Canonical path comparison.** Paths in both `descriptor.entries[]` and tar headers MUST decode as UTF-8 and MUST already be in NFC. Validation rejects any descriptor or tar path for which NFC normalization would change the string; installers MUST NOT "repair" non-NFC input by normalizing it during install (normalization is a producer-side responsibility, enforced at the installer boundary only as rejection). After that validation, descriptor paths are compared against tar entry names using the already-validated bytes, forward-slash separators, and explicit rejection of any `./` prefix, trailing slash, or empty segment. No ambiguity between `payloads/x/y` and `./payloads/x/y`; no path can mean different things on different platforms.
...
- Payload-type framing: `application/vnd.mship.bundle-descriptor+json`. The signed bytes are DSSE's standard PAE encoding — see the normative byte-sequence definition in §"What is signed" below. Summary form: `b"DSSEv1 <ascii-decimal-len(payload_type)> <payload_type> <ascii-decimal-len(payload)> <payload>"`, where each space is a literal ASCII 0x20 separator and each length is the ASCII decimal byte-count of the following field. §"What is signed" is normative; this summary is informational.
...
Install verification when `identity_method: sigstore` (the signature-verification sub-steps of the `descriptor-verified` state):
...
- Verify the cert was valid at `integrated_time_unix` from the Rekor entry (not at install-time wall clock).
...
Install is a **journaled state machine**, not a sequence of ad-hoc steps. Each state transition is durable, idempotent, and crash-recoverable; install flow audit events are emitted for the defined install-event milestones declared later in this spec (`plugin.install-requested`, `plugin.signature-verified`, `plugin.signature-failed`, `plugin.install-approved`, `plugin.install-rejected`, `plugin.uninstalled`). Only one terminal state (`activated`) makes the bundle's manifests observable to Broker (T3) and Skill process isolation (T4).
...
The install state machine ends at `activated` (or a terminal `rejected`). **Post-install bundle lifecycle** — deactivation on revocation / operator uninstall, grace-period retention, deletion, and re-activation — is a **separate bundle-lifecycle state machine** owned by §"Revocation — deactivate first, delete later". It operates on already-installed bundles (those that reached `activated`) and is not part of the install journal: transitions there are recorded directly in the audit log via `plugin.uninstalled` / `plugin.revocation-impact-reported` / re-activate audit events, not as `journal.jsonl` entries for a specific install_event_id.
...
**`received`** — the `.mship` archive has been placed in a quarantine directory at `installs/<install_event_id>/archive.mship`. No further files have been written. A `plugin.install-requested` audit event is emitted. The archive is not yet trusted.
...
- Archive parse: stream-parse tar entries, rejecting any non-regular-file entry, any disallowed path shape, any duplicate path. Every accepted tar entry is hashed on-the-fly.
- **Required-metadata structural check** (new, `received`-state responsibility): assert the archive contains **exactly one** `descriptor.json` entry and **exactly one** `signature.dsse` entry at the archive root, and that any other non-metadata entry is under `payloads/**`. Missing, duplicate, or misplaced required metadata entries → `rejected(descriptor-tar-mismatch)`. Anything outside `payloads/**` that is not `descriptor.json` or `signature.dsse` → `rejected(tar-structural)`. This check must pass before the extraction step below.
- Only two files are extracted to memory: `descriptor.json` and `signature.dsse`. Payload files are NOT written to disk yet; their hashes are retained for the next state.
...
- Namespace check: `publisher_id` **exactly matches** an entry in the chain-verifying root's `owned_publisher_ids` (or in the matched sigstore identity allowlist entry's `owned_publisher_ids`). → `rejected(publisher-namespace-not-owned)`.
...
- **Verify every tar file under `payloads/**` has exactly one matching object in `descriptor.entries[]` by `(path, size, hash)`, and vice-versa** (descriptor↔tar bijection, payloads only; the two structural metadata entries `descriptor.json` + `signature.dsse` were already validated in `received` and are explicitly excluded here); any extra payload tar entry OR any descriptor entry without a matching payload tar entry → `rejected(descriptor-tar-mismatch)`.

(The "exactly one `descriptor.json` + one `signature.dsse` at archive root" and "no non-metadata non-payload entries" structural checks are performed in the `received` state above; by the time `descriptor-verified` runs, those structural preconditions are already satisfied.)
...
- Any per-entry content mismatch (size, SHA-256, or disallowed media_type) → `rejected(entry-hash-mismatch)`; all partially-materialized files are deleted; the tarball stays in quarantine for operator inspection. A tar-header `mode` differing from the descriptor's declared `mode` does **not** abort install — mode mismatches are normalized during the `staged → committed` transition, not in `staged` itself.
...
- `plugin.signature-verified` — `{install_event_id, bundle_hash, publisher_id, algorithm, publisher_cert_fingerprint, identity_method, trust_root_fingerprint?, sigstore_identity_subject?, rekor_entry_uuid?}` — field name matches `signature.dsse`'s `verification_material.rekor.entry_uuid` exactly for log-correlation unambiguity.
...
- Rejected: absolute paths, `..`, single-`./` prefixes, trailing slashes, empty path segments, path > 1024 bytes, Windows drive letters, UNC prefixes, NTFS ADS colons, hardlinks, symlinks, FIFOs, device files, sparse files, any pax extended-header entry (tar type `x`), any global pax header (tar type `g`), extended attributes, non-NFC-normalized Unicode paths (rejected, not auto-normalized), duplicate paths (exact-byte after NFC check), case-insensitive collisions on HFS+/NTFS hosts, zero-byte files where descriptor declares non-zero size, non-regular-file entry types.
...
- Build tool must reject ambiguous authoring inputs (duplicate keys, visually-confusable publisher_id vs. trust-root `owned_publisher_ids` entries).

**Cert chain + namespace:**

- Self-signed operator-added root → passes.
- Cert chain up to an un-listed root → `chain-untrusted`.
- Chain valid but `publisher_id` not exactly listed in the matched root's `owned_publisher_ids` → `publisher-namespace-not-owned`.
- Two trust roots in the store both listing the same `publisher_id` in their `owned_publisher_ids` → `mothership trust add` refuses to add the second; if added out-of-band, install aborts with `trust-store-namespace-collision`.
...
1. **Unsigned bundle** (missing `signature.dsse`) → `descriptor-tar-mismatch` (missing required metadata entry is rejected as a structural violation during `received` by the required-metadata structural check, before any signature-verification attempt, per the `received`-state contract).
...
13. **Publisher-namespace collision attempt** (cert chains to a root, but `publisher_id` is not exactly listed in that root's `owned_publisher_ids`) → `publisher-namespace-not-owned`.
...
| P7 | After installing Mothership and N skills with no explicit grants, can any skill read filesystem, open shell, access secrets, or make external network requests? | **No.** In the `reviewed` state, every grant remains off by default until the operator explicitly enables it. Trust level is badge-only in V1 and does not affect grant defaults or suggestions. |
| P8 | Do install-related events produce audit entries? | **Yes.** Seven declared event types covering request/rejected/approved/signature-verified/signature-failed/uninstalled/revocation-impact-reported, with typed payload hygiene. |
| P2 | Can a compromised agent cause bundle install? | **No.** Install is supervisor-owned; operator-presence-gated re-auth happens in the `reviewed` state. No agent-addressable API can trigger install or grant changes. |
...
- **Trust-level badges vs. default-deny.** Even badge-only trust signals (for example, `verified` or `organization`) could bias operators toward enabling grants more readily on a "nicer-looking" bundle than on a `community` one. Mitigation: in V1 trust level does not change grant defaults, does not pre-suggest grants, and does not flag any capability as "recommended"; the grant UI requires at least one click per grant (no "approve all" button); every grant is individually logged; operators habituated to rubber-stamping are a UX problem beyond substrate scope but the substrate at least makes the decisions auditable. Revisit pre-suggestion UX in V1.5 only with design-partner evidence that richer UI does not erode operator decision quality.
...
| 2026-04-19 | V0.10 ninth-round Copilot fixes. | Fixed 5 more issues on PR #1: (a) three **stale `publisher_namespace_prefixes` references** caught (namespace-check step, L2 cert-chain tests, L4 acceptance test #13) — leftover from V0.9's rename to `owned_publisher_ids`, now all aligned; (b) **path normalization reconciled** — invariant was internally inconsistent ("reject non-NFC at parse" vs "compare after NFC normalization"). New rule: both descriptor and tar paths MUST already be NFC; non-NFC is rejected at parse; installers MUST NOT auto-normalize during install. Comparison uses already-validated bytes. Matches audit log's pattern of rejecting rather than repairing malformed input; (c) **`bundle_id` ↔ `publisher_id` consistency check made explicit** — the grammar required `bundle_id = <publisher_id>/<local_name>` but never explicitly said the verifier must check the first segment of `bundle_id` equals the standalone `publisher_id` field. Now the verifier MUST split on `/` and require byte-exact match; neither field is authoritative over the other; build tooling MUST fail generation (not silently choose one) if they differ. Closes a "which field is the source of truth" ambiguity that could have caused divergent implementations. No contract changes to downstream sub-specs. |
